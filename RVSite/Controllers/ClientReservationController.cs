...
namespace RVSite.Controllers
{
    public class ClientReservationController : Controller
    {
            ...
            bool search = false)
        {
            ...
            {
                TempData["ErrorMessage"] =
                    "Please confirm your email before booking a reservation.";
            ...
            if (search)
            {
                ...
                ModelState.Remove(
                    nameof(Reservation.SiteID));

                ModelState.Remove(
                    nameof(Reservation.NumberOfAdults));

                ModelState.Remove(
                    nameof(Reservation.NumberOfChildren));

                ModelState.Remove(
                    nameof(Reservation.NumberOfPets));

                ModelState.Remove(
                    nameof(Reservation.SpecialRequests));
                ...
                var availableSites =
                    _context.Sites
                        .Where(s =>
                            s.SiteType.Name == siteType &&
                            s.SiteStatus !=
                                SiteStatus.Maintenance.ToString() &&
                            (rvLength == null ||
                             s.MaxRVLength >= rvLength))
                        .Where(s =>
            ...
            var policyValidation =
                await _reservationPolicyService
                    .ValidateReservationAsync(
                        model.CheckInDate,
                        model.CheckOutDate,
                        model.UserID,
                        model.SiteID);
            ...
            model.TotalCost =
                _costService.CalculateCost(model);
            ...
            int? rvLength)
        {
            ...
            if (!availableSites.Any())
            {
                ModelState.AddModelError(
                    "",
                    "No sites available for the updated criteria.");
        ...
        // =====================================================
        // CUSTOMER CANCELLATION - CONFIRMATION PAGE
        // =====================================================
        ...
        public async Task<IActionResult> Cancel(int id)
        {
            ...
            // If reservation was already cancelled,
            // display the result page instead
            ...
            // This displays Cancel.cshtml first.
            return View(reservation);
        }


        // =====================================================
        // CUSTOMER CANCELLATION - ACTUAL CANCELLATION
        // =====================================================
            ...
            int id)
        {
            ...
            // If already cancelled, do not process again
            if (reservation.ReservationStatus ==
                ReservationStatus.Cancelled)
            {
                return View(
            ...
            // Apply cancellation fee based on policy
            await AddCancellationFeeAsync(
                reservation);

            // Check if another reservation still
            // requires this campsite
            ...
            // Make campsite available again
            // unless another reservation needs it
            ...
            // adding possible cancellation fee
            await _costService
                .UpdateReservationBalanceDueAsync(
                    reservation.ReservationID);

            await _context.SaveChangesAsync();

            // Display final cancellation result page
            return View(
                "CancelConfirmation",
                reservation);
        }
        ...
        private async Task AddCancellationFeeAsync(
            Reservation reservation)
        {
            bool cancellationFeeApplies =
                await CancellationFeeAppliesAsync(
                    reservation);
            ...
            bool cancellationFeeAlreadyExists =
                await _context.Fees.AnyAsync(f =>
                    f.ReservationID ==
                        reservation.ReservationID &&
                    f.NameCode ==
                        FeeCodes.Cancellation);
